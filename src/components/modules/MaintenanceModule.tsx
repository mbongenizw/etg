...
import { Wrench, Plus, Edit, Trash2, Search, Eye, Car, Calendar, AlertCircle, ChevronLeft, ChevronRight, Check, X } from 'lucide-react';
...
export function MaintenanceModule() {
  ...
  const [formData, setFormData] = useState<Partial<MaintenanceRecord> & { types: string[] }>({ types: [] });
  ...
  const fetchRecords = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Filter for maintenance types only
        const maintenanceRecords = data.filter((r: MaintenanceRecord) => 
          r.types && r.types.some((t: string) => maintenanceTypes.includes(t))
        );
  ...
  const handleSave = async () => {
    if (!formData.vehicleId || formData.types?.length === 0 || !formData.description || !formData.startDate) {
      toast.error('Please fill in all required fields and select at least one maintenance type');
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        toast.success(selectedRecord ? 'Maintenance record updated' : 'Maintenance record created');
        handleCloseDialog();
        fetchRecords();
      } else {
        const data = await res.json();
  ...
  const getStatusBadge = (status: string) => {
    const config = maintenanceStatuses.find(s => s.value === status);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-amber-900">Maintenance</h1>
          <p className="text-gray-600">Manage vehicle maintenance and repair records</p>
        </div>
        ...
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 py-4">
            ...
            <div className="space-y-2 sm:col-span-2">
              <Label>Maintenance Types * (Select one or more)</Label>
            ...
            <div className="space-y-2 sm:col-span-2">
              ...
              <Input id="description" value={formData.description || ''} onChange={(e) => setFormData({ ...formData, description: e.target.value })} placeholder="Maintenance description" />
            </div>
            <div className="space-y-2">
